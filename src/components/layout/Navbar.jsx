...
import { Shield, Sun, Moon, Menu, X, LogOut, User, ChevronDown } from 'lucide-react';
...
export default function Navbar() {
  const { usuario, cerrarSesion } = useAuth(); // Se asume que tienes cerrarSesion en tu AuthContext
  const { tema, alternarTema } = useTema();
  const location = useLocation();

  // Estados para la respuesta interactiva y móvil
  const [menuMovilAbierto, setMenuMovilAbierto] = useState(false);
  const [menuUsuarioAbierto, setMenuUsuarioAbierto] = useState(false);
  const [isMobile, setIsMobile] = useState(window.innerWidth < 900);

  // Monitorear el ancho de pantalla para cambiar dinámicamente entre escritorio y móvil
  useEffect(() => {
    const handleResize = () => {
      const mobile = window.innerWidth < 900;
      setIsMobile(mobile);
      if (!mobile) setMenuMovilAbierto(false); // Cierra el menú móvil si se agranda la pantalla
    };
  ...
  // Estilo dinámico para los enlaces de navegación
  ...
  return (
    <nav style={{ 
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        {/* LOGO */}
        <Link to="/" style={{ textDecoration: 'none', display: 'flex', alignItems: 'center', zIndex: 1001 }}>
          <img 
            src={tema === 'light' ? logoDark : logoLight} 
            alt="JFS Logo" 
            style={{ height: isMobile ? '34px' : '40px', width: 'auto', transition: 'transform 0.3s' }} 
          />
        </Link>
            ...
            {usuario ? (
              <div style={{ position: 'relative', marginLeft: '10px' }}>
                <button 
                  ...
                  style={{ background: 'var(--bg-secondary)', border: `1px solid var(--border)`, padding: '8px 14px', borderRadius: '12px', cursor: 'pointer', color: 'var(--text-main)', display: 'flex', alignItems: 'center', gap: '8px', fontWeight: '600', fontSize: '14px' }}
                >
                ...
                {menuUsuarioAbierto && (
                  <div style={{ position: 'absolute', top: 'calc(100% + 8px)', right: 0, background: 'var(--bg-primary)', border: '1px solid var(--border)', borderRadius: '14px', padding: '8px', width: '200px', boxShadow: '0 10px 25px rgba(0,0,0,0.1)' }}>
                    ...
                    <button onClick={cerrarSesion} style={{ width: '100%', background: 'transparent', border: 'none', padding: '10px 12px', borderRadius: '10px', color: 'var(--danger)', display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer', fontSize: '14px', fontWeight: '500', textAlign: 'left' }}>
        ...
        {isMobile && (
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', zIndex: 1001 }}>
            ...
            <button 
              ...
              style={{ background: 'var(--bg-secondary)', border: '1px solid var(--border)', color: 'var(--text-main)', padding: '8px', borderRadius: '10px', cursor: 'pointer', display: 'flex', alignItems: 'center' }}
            >
      ...
      {isMobile && menuMovilAbierto && (
        <div style={{ 
          ...
          background: 'var(--bg-primary)', padding: '90px 24px 24px 24px', 
          ...
          {usuario ? (
            <>
              ...
              <button onClick={cerrarSesion} style={{ width: '100%', background: 'var(--bg-secondary)', border: '1px solid var(--border)', padding: '14px 20px', borderRadius: '12px', color: 'var(--danger)', display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer', fontSize: '14px', fontWeight: '600' }}>
                <LogOut size={16} /> Cerrar Sesión
              </button>
            </>
          ) : (
